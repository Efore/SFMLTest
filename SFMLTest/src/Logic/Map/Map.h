...
namespace Logic
{
	...
	*/
	...
	{
	public:
		Map(const unsigned int& colums, const unsigned int& rows, sf::Texture& texture);
		~Map();
		...
		const bool& GetValueAt(const sf::Vector2f& Position);
		...
		Entity* GetTileAt(const sf::Vector2f& Position);
	...
	private:
		Entity * CreateTileEntity(const unsigned int & column, const unsigned int & row, const bool& validPosition, sf::Texture & texture);

	private:
		unsigned int m_mapRows;
		unsigned int m_mapColums;
